...
# 1. Extract the fragment shader from GhostFibers.jsx
...
js_code = f"""import {{ Renderer, Program, Triangle, Mesh }} from 'https://cdn.jsdelivr.net/npm/ogl@0.0.117/dist/ogl.mjs';
...
export function initGhostFibers(container, options = {{}}) {{
  ...
  const loop = now => {{
    ...
    if (now - lastRenderTime >= 1000 / fps - 0.5) {{
...
with open('js/ghost-fibers.js', 'w', encoding='utf-8') as f:
    f.write(js_code)

# 2. Modify index.html to load and instantiate GhostFibers
...
if 'import { initGhostFibers }' not in html:
    html = html.replace('</body>', script_tag)
    
    # Optional: also remove existing hero-bg styles if any, but since we inserted our own it's fine.
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html)
    print("Injected GhostFibers into index.html")
else:
    print("GhostFibers script already present in index.html")

print("GhostFibers setup complete.")
